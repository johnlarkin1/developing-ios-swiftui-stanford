...
struct FilterFlights: View {
    ...
    // Need this to be a binding!!
    var destination: Binding<MKAnnotation?> {
        // very primitive / old school getter and setter
        return Binding<MKAnnotation?>(
            get: { return self.draft.destination },
            set: { annotation in
                // annotation here is an MKAnnotaiton? so we need that 
        ...
        NavigationView {
            Form {
                Section {
                    ...
                    /*
                     THIS IS THE CODE WE ADD IN Lecture 14
                     */
